...
export default function WalletConnect() {
  ...
  if (!connected) {
    return (
      <div className="relative inline-flex items-center gap-2">
        ...
        <span className="text-sm text-gray-400 px-2 py-1 border border-white/15 rounded-md select-none">
          {network}
        </span>
      </div>
    );
  ...
  return (
    <div className="relative inline-flex items-center gap-2">
      ...
      >
        <span className="inline-block w-2 h-2 rounded-full bg-green-500 mr-2 shadow-[0_0_0_3px_rgba(76,175,80,0.15)]" />{" "}
        {shortAddr(publicKey)}
      </button>
      <span className="text-sm text-gray-400 px-2 py-1 border border-white/15 rounded-md select-none">
        {network}
