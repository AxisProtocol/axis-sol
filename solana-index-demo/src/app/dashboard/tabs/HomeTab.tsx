...
const ClaimModal = dynamic(
  () => import("../../../components/modals/ClaimModal"),
  { ssr: false }
);
...
const HomeTab = ({ echartsData }: HomeTabProps) => {
  ...
  return (
    <div className="space-y-6 sm:space-y-8 pt-16">
      {/* Header */}
      <div className="text-center">
        ...
        <p className="text-base-content/60 text-sm sm:text-base">
          Welcome to the AXIS Protocol Dashboard
        </p>
      </div>
        ...
        {featureCards.map((card, index) => (
          ...
          >
            <div className="text-center">
              ...
              <p className="text-base-content/60 text-xs sm:text-sm mb-2 sm:mb-3">
                {card.subtitle}
              </p>
              <p className="text-base-content/70 text-xs mb-3 sm:mb-4">
                {card.description}
              </p>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button className="text-orange-400 text-xs sm:text-sm hover:text-orange-300 transition-colors">
            Discover more{" "}
            <ArrowRight className="inline w-4 h-4 ml-1 align-middle" />
          </button>
        </div>
          ...
          {favorites.length > 0 ? (
              ...
              {favorites.map((favorite, index) => (
                ...
                >
                  <div className="flex-1 min-w-0">
                    <h3 className="text-base-content font-medium text-sm sm:text-base truncate">
                      {favorite.name}
                    </h3>
                  ...
                  <span className="text-base-content/60 ml-2 flex-shrink-0">
                    →
                  </span>
                </div>
              ))}
            ...
            <div className="text-center py-8">
              <p className="text-base-content/60">
                ...
                <span className="text-secondary cursor-pointer hover:opacity-80">
                  add some!
                </span>
              </p>
            </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 pb-15">
        <ModernCard className="p-3 sm:p-4 text-center" gradient>
          ...
          <div className="text-base-content/70 text-xs sm:text-sm">
            Current Index Price (normalized)
          </div>
        </ModernCard>
        <ModernCard className="p-3 sm:p-4 text-center" gradient>
          <Coins className="w-5 h-5 sm:w-6 sm:h-6 mx-auto mb-2" />
